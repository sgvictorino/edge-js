...
function determineVersion(parentDirectory) {
    const latestCompatibleVersion = fs.readdirSync(parentDirectory).filter((version) => version.match(new RegExp(`^${process.versions.node.split(".")[0]}\.`))).sort().slice(-1)[0];
    if (typeof latestCompatibleVersion === "string") {
...
if (process.env.EDGE_NATIVE) {
    ...
    const versionsDirectory = path.resolve(__dirname, './native/' + process.platform + '/' + process.arch);
    edgeNative = path.join(versionsDirectory, determineVersion(versionsDirectory), process.env.EDGE_USE_CORECLR ? 'edge_coreclr' : 'edge_nativeclr');
}
else {
